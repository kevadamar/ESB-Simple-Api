...
class TBillingInvoicesService extends BaseService
{
    ...
    public function storeInvoice(array $payload = [])
    {
        ...
        foreach ($payload['detail'] as $key => $value) {
            ...
            $payloadInvoiceDetails['m_service_id'] = $value['m_service_id'];
            $payloadInvoiceDetails['quantity'] = $value['quantity'];
            $payloadInvoiceDetails['total_subamount'] = $subTotalAmount;
            $payloadInvoiceDetails['created_at'] = Carbon::now();
            $payloadInvoiceDetails['updated_at'] = Carbon::now();
        ...
        foreach ($payloadInvoiceDetails as $key => $value) {
            $payloadInvoiceDetails['t_billing_invoice_id'] = $store->id;
        }
    ...
    public function updateInvoice(array $payload = [], $id = null)
    {
        ...
        foreach ($payload['detail'] as $key => $value) {
            ...
            $this->update($this->tBillingInvoiceDetails, $payloadInvoiceDetails, $value['id']);
